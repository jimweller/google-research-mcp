...
import {
  PeriodicPersistenceStrategy,
  WriteThroughPersistenceStrategy,
  OnShutdownPersistenceStrategy,
  HybridPersistenceStrategy
} from './persistenceStrategies.js';
...
describe('PersistenceStrategies', () => {
  ...
  describe('PeriodicPersistenceStrategy', () => {
    ...
    it('should filter namespaces when configured', () => {
      ...
      expect((strategy as any).shouldPersistNamespace('persistent')).toBe(true);
      expect((strategy as any).shouldPersistNamespace('non-persistent')).toBe(false);
    });

    it('should persist all namespaces when none are specified', () => {
      const strategy = new PeriodicPersistenceStrategy(60000);
      
      expect((strategy as any).shouldPersistNamespace('any-namespace')).toBe(true);
    });
  });
  ...
  describe('HybridPersistenceStrategy', () => {
    it('should persist critical namespaces on set', () => {
      const strategy = new HybridPersistenceStrategy(
        ['critical'],  // Critical namespaces
    ...
    it('should respect both critical and persistent namespace lists', () => {
      const strategy = new HybridPersistenceStrategy(
        ['critical1', 'critical2'],  // Critical namespaces
        60000,                       // Persistence interval
    ...
    it('should handle empty critical namespaces list', () => {
      const strategy = new HybridPersistenceStrategy(
        [],  // No critical namespaces
        60000,
        ['persistent']  // Only persistent namespaces
      );
