...
describe('PersistentEventStore', () => {
  let eventStore: PersistentEventStore;
  let mockPersistenceManager: jest.Mocked<EventPersistenceManager>;
  let mockNow: number;
  ...
  beforeEach(() => {
    ...
    mockPersistenceManager = {
      ...
      dispose: jest.fn()
    } as unknown as jest.Mocked<EventPersistenceManager>;
    
    // Replace the internal persistence manager with our mock
    (eventStore as any).persistenceManager = mockPersistenceManager;
  ...
  describe('storeEvent', () => {
    it('should store an event and return an event ID', async () => {
      ...
      const message: JSONRPCMessage = { jsonrpc: "2.0", method: 'test', params: { data: 'test' } };
    ...
    it('should persist critical stream events immediately', async () => {
      ...
      // Replace the internal persistence manager with our mock
      (criticalEventStore as any).persistenceManager = mockPersistenceManager;
      
      // Mock persistEvent to resolve successfully
      mockPersistenceManager.persistEvent.mockResolvedValue();
      
      // Need to cast message to any to bypass TypeScript's type checking
      const eventId = await criticalEventStore.storeEvent(streamId, message as any);
      
      // Check that persistEvent was called for critical stream
      expect(mockPersistenceManager.persistEvent).toHaveBeenCalledWith(
        eventId,
  ...
  describe('replayEventsAfter', () => {
    it('should replay events after a given event ID', async () => {
      ...
      try {
        const streamId = 'test-stream';
        const message1: JSONRPCMessage = { jsonrpc: "2.0", method: 'test1', params: { data: 'test1' } };
        const message2: JSONRPCMessage = { jsonrpc: "2.0", method: 'test2', params: { data: 'test2' } };
        const message3: JSONRPCMessage = { jsonrpc: "2.0", method: 'test3', params: { data: 'test3' } };
        ...
        const mockSend = jest.fn<(eventId: string, message: JSONRPCMessage) => Promise<void>>()
  ...
  describe('persistToDisk', () => {
    it('should persist all events to disk', async () => {
      ...
      expect(mockPersistenceManager.persistEvents).toHaveBeenCalledWith(
        expect.any(Map)
      );
    });
  });
