...
export class EventPersistenceManager {
  ...
  private onPersistCallback?: () => Promise<Map<string, EventData>>;
  ...
  async persistEvents(events: Map<string, EventData>): Promise<void> {
    ...
    const eventsByStream = new Map<string, Map<string, EventData>>();
    ...
    for (const [streamId, streamEvents] of Array.from(eventsByStream.entries())) {
      ...
      for (const [eventId, data] of Array.from(streamEvents.entries())) {
        ...
        persistPromises.push(
          this.writeEventFile(eventPath, data)
        );
      }
    }
   ...
   */
  async persistEvent(eventId: string, data: EventData): Promise<void> {
   ...
   */
  async loadEvent(eventId: string): Promise<EventData | null> {
   ...
   */
  private async onPersistenceInterval(): Promise<void> {
    // Skip if disposed or no callback
    if (this.isDisposed || !this.onPersistCallback) return;
   ...
   */
  private async ensureDirectoryExistsAsync(dir: string): Promise<void> {
    try {
   ...
   */
  private async writeEventFile(filePath: string, data: EventData): Promise<void> {
   ...
   */
  private async readEventFile(filePath: string): Promise<EventData> {
