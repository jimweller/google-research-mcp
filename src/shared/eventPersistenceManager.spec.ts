...
describe('EventPersistenceManager', () => {
  ...
  describe('persistEvent and loadEvent', () => {
    it('should persist and load an event', async () => {
      ...
      const eventData: EventData = {
        streamId,
        message: { jsonrpc: "2.0" as const, method: 'test', params: { data: 'test' } },
        timestamp: Date.now(),
  ...
  describe('persistEvents and loadEvents', () => {
    it('should persist and load multiple events', async () => {
      ...
      const eventData1: EventData = {
        streamId: streamId1,
        message: { jsonrpc: "2.0" as const, method: 'test1', params: { data: 'test1' } },
        timestamp: Date.now()
      };
      
      const streamId2 = 'stream2';
      const eventId2 = `${streamId2}_${Date.now()}_def`;
      const eventData2: EventData = {
        streamId: streamId2,
        message: { jsonrpc: "2.0" as const, method: 'test2', params: { data: 'test2' } },
        timestamp: Date.now()
      };
  ...
  describe('onPersistenceInterval', () => {
    ...
    it('should call the persistence callback when the interval fires', async () => {
      ...
      const mockCallback = jest.fn<() => Promise<Map<string, EventData>>>()
        .mockResolvedValue(new Map<string, EventData>());
      ...
      await (persistenceManager as any).onPersistenceInterval();
  ...
  describe('calculateDiskUsage', () => {
    it('should calculate the disk usage of all events', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const eventData: EventData = {
          streamId,
          message: { jsonrpc: "2.0" as const, method: 'test', params: { data: `test${i}` } },
