...
- **Academic Paper Search**: New `academic_search` tool searches peer-reviewed papers via Semantic Scholar API (free tier, no API key required). Returns titles, authors, abstracts, citation counts, PDF URLs, and pre-formatted citations (APA, MLA, BibTeX). Papers cached for 24 hours since they don't change. (#74)
