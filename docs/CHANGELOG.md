...
### Added
- **Patent Assignee Search Tool**: New `patent_assignee_search` tool searches all patents for a specific company/assignee using the PatentsView API (separate from Google Custom Search). Features: full pagination (up to 100 results/page), patent status calculation (active/expired), technology distribution by CPC codes, summary aggregations by status/type/year. Requires free PatentsView API key. (#80)
- **Patent Details Tool**: New `patent_details` tool retrieves comprehensive details for a specific patent from PatentsView API. Features: full metadata (title, abstract, dates), assignee and inventor details with locations, CPC classification codes with descriptions, citation data (forward/backward), claims text, calculated expiration date and status. Accepts multiple patent number formats (e.g., "11886826", "US11886826", "US11886826B1"). (#80)
- **Patent Portfolio Analysis Prompt**: New `patent-portfolio-analysis` MCP prompt provides guided workflow for comprehensive patent portfolio analysis. Analyzes portfolio size, technology areas, patent status distribution, and suggests competitor comparison. (#80)
- **Shared Patent Constants**: New `src/shared/patentConstants.ts` centralizes patent-related constants and helper functions: CPC section descriptions, assignee type mappings, patent office prefixes, patent term calculations, and expiration/status helpers. Eliminates code duplication across patent tools.
...
### Changed
- **Improved LLM Tool Descriptions**: Standardized all 10 tool descriptions with consistent "When to use" format. Added missing caching information to image/news/academic/patent tools. Improved google_search description to focus on standalone use cases. Makes LLM tool selection more accurate and efficient.
